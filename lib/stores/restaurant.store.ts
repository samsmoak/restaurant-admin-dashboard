...
import { adminRestaurantApi, restaurantsApi } from '@/lib/api/endpoints';
...
type RestaurantState = {
  ...
  createRestaurant: (input: { slug: string; name: string; description?: string; phone?: string; email?: string; address?: string }) => Promise<GoRestaurant>;
  update: (body: unknown) => Promise<void>;
...
export const useRestaurantStore = create<RestaurantState>((set) => ({
  ...
  async createRestaurant(input) {
    ...
    try {
      ...
      set((s) => ({ mine: [r, ...s.mine], loading: false }));
      return r;
    } catch (e) {
